...
deriv(A[2:na, 1:nh,1:2]) <- (1-phi[i,j,k])*FOI[i,j,k]*Y[i,j,k] - FOI[i,j,k]*A[i,j,k] +
...
deriv(U[2:na, 1:nh,1:2]) <- rA*A[i,j,k] - FOI[i,j,k]*U[i,j,k] - rU*U[i,j,k] -
...
clin_inc[1:na, 1:nh,1:2] <- phi[i,j,k]*FOI[i,j,k]*Y[i,j,k]
...
deriv(ICA[1, 1:nh,1:2]) <- FOI[i,j,k]/(FOI[i,j,k] * uCA + 1) - 1/dCA*ICA[i,j,k] -ICA[i,j,k]/x_I[i]
deriv(ICA[2:na, 1:nh,1:2]) <- FOI[i,j,k]/(FOI[i,j,k] * uCA + 1) - 1/dCA*ICA[i,j,k] - (ICA[i,j,k]-ICA[i-1,j,k])/x_I[i]
...
deriv(ID[1, 1:nh,1:2]) <- FOI[i,j,k]/(FOI[i,j,k]*uD + 1) - ID[i,j,k]/dID - ID[i,j,k]/x_I[i]
deriv(ID[2:na, 1:nh,1:2]) <- FOI[i,j,k]/(FOI[i,j,k]*uD + 1) - ID[i,j,k]/dID - (ID[i,j,k]-ID[i-1,j,k])/x_I[i]
...
#dim(FOI_lag) <- c(na,nh)
#FOI_lag[1:na, 1:nh] <- EIR[i,j,k] * (if(IB[i,j,k]==0) b0 else b[i,j,k])
...
EIR[,,2] <- EIR_td*rel_foi[j] * foi_age[i]*SMC_prot
#output(Ivout) <- Iv
...
#FOIvij[1:na, 1:nh] <- (cT*T[i,j,k] + cD*D[i,j,k] + cA[i,j,k]*A[i,j,k] + cU*U[i,j,k]) *av* rel_foi[j] *foi_age[i]/omega
#lag_FOIv=sum(FOIvij)
